...
控制命令必须以 `/` 开头并与整条消息完全匹配。任何以 `/` 开头但未匹配的消息都会直接返回“命令不存在”，不会发送给 ChatGPT 或 Codex，也不会发送“正在处理”确认。不带 `/` 的“状态”“重试”等文字仍会作为普通聊天发送给 ChatGPT。

- `/帮助`（或 `/发送帮助`）：列出所有命令、格式和描述。
- `/新对话`（或 `/清理上下文`）：下一条消息创建新对话，旧对话保留。
- `/当前对话`：查看当前对话标题和 ChatGPT 地址。
- `/对话列表`、`/切换对话：2`：列出并恢复当前微信用户的历史对话。
- `/归档对话`、`/重命名对话：标题`：归档或重命名当前对话。
- `/导出对话`：抓取当前对话并生成 Markdown，以微信文件附件发送。
- `/总结对话`：让 ChatGPT 总结完整上下文，成功后开启新对话。
- `/重试`：重新处理上一条问题；图片任务会要求重新生成。
- `/重发`：不请求 ChatGPT，重发最后一次成功回复。
- `/干活：任务`、`/干活 项目名：任务`：让 Codex 在当前或指定项目执行。
- `/继续：要求`、`/最近任务`：继续 Codex 会话或查看最近任务及结果。
- `/项目列表`、`/切换项目：项目名`：查看或切换当前微信用户的默认 Codex 项目。
- `/状态`、`/停止`：查看或停止当前任务。
- `/缓存状态`、`/清理缓存：7天`：查看或清理图片缓存。
- `/查看日志：50`：返回最近 50 条脱敏日志，范围为 1 到 200 条。
- `/健康检查`：查看微信连接、ChatGPT 浏览器、Codex 和缓存状态。
- `/重连微信`、`/重启浏览器`：重新建立微信连接或重启隐藏浏览器。
