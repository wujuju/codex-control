...
# Chinese command names are also available through their pinyin initials.
_PINYIN_COMMANDS = {
    "bz": "帮助",
    "fsbz": "发送帮助",
    "xdh": "新对话",
...
_PINYIN_PREFIX = re.compile(r"^@([a-z]+)(?=$|[：:\s])", re.IGNORECASE)


def _normalize_pinyin_command(text: str) -> str:
    match = _PINYIN_PREFIX.match(text)
    if not match:
        return text
    command = _PINYIN_COMMANDS.get(match.group(1).casefold())
    if command is None:
        return text
    return f"@{command}{text[match.end():]}"


def route_message(text: str) -> Route:
    cleaned = _normalize_pinyin_command(text.strip())
    if cleaned == "@状态":
...
def help_text(projects: list[str], default_project: str) -> str:
    ...
    return (
        "命令帮助（必须以 @ 开头并完整匹配；支持拼音首字母且不区分大小写）\n"
        "\n【ChatGPT 对话命令】\n"
        "@新对话（@xdh）：开启新对话，旧对话保留\n"
        "@当前对话（@dqdh）：查看当前对话\n"
        "@对话列表（@dhlb）：列出历史对话\n"
        "@切换对话：2（@qhdh：2）：切换历史对话\n"
        "@归档对话（@gddh）：归档当前对话\n"
        "@重命名对话：标题（@cmmdh：标题）：修改标题\n"
        "@导出对话（@dcdh）：导出 Markdown\n"
        "@总结对话（@zjdh）：总结后开启新对话\n"
        "@重试（@cs）：重新回答上一条问题\n"
        "@重发（@cf）：重发最后一次成功回复\n"
        "\n【Codex 开发命令】\n"
        f"@干活：任务（@gh：任务）：修改当前项目（默认 {default_project}）\n"
        "@干活 项目名：任务（@gh 项目名：任务）：修改指定项目\n"
        "@继续：要求（@jx：要求）：继续最近一次 Codex 会话\n"
        "@最近任务（@zjrw）：查看最近任务与结果\n"
        "@项目列表（@xmlb）：查看项目和当前选择\n"
        "@切换项目：项目名（@qhxm：项目名）：切换当前项目\n"
        "\n【通用与维护命令】\n"
        "@状态（@zt）：查看当前任务状态\n"
        "@停止（@tz）：停止当前任务\n"
        "@缓存状态（@hczt）、@清理缓存：7天（@qlhc：7天）\n"
        "@查看日志：50（@ckrz：50）、@健康检查（@jkjc）\n"
        "@重连微信（@clwx）、@重启浏览器（@cqllq）\n"
        "@帮助（@bz）：显示本帮助\n"
        f"\n可用项目：{project_text}\n"
        "未知命令会提示最接近的正确命令。"
    )
